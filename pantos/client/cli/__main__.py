...
from pantos.client.cli.application import initialize_application
from pantos.client.cli.configuration import config
from pantos.client.cli.configuration import get_blockchain_config
from pantos.client.cli.exceptions import ClientCliError
...
def _load_private_key(
    ...
    return api.load_private_key(blockchain, keystore_path, password)


def _print_balance(blockchain: api.Blockchain, token_symbol: api.TokenSymbol,
                   balance: decimal.Decimal) -> None:
    print(f'Your {token_symbol.upper()} token balance on {blockchain.name}:\n'
          f'{balance}')
...
def _print_transfer_inputs(source_blockchain: api.Blockchain,
    ...
    print(f'Source blockchain:\t{source_blockchain.name}')
    print(f'Destination blockchain:\t{destination_blockchain.name}')
    print(f'Recipient ({destination_blockchain.name}):\t{recipient_address}')
    print(f'Token symbol:\t\t{token_symbol.upper()}')
    print(f'Amount:\t\t\t{amount}')
    print(f'Keystore ({source_blockchain.name}):\t'
